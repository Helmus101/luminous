...
function App() {
  ...
  const handleWarmIntro = (campusName: string) => {
    setActiveTab('chat')
    setChatInput(`I'd like to connect with a scout from ${campusName}.`)
  }
      ...
      {activeTab === 'chat' ? (
        ...
        <div className="discovery-view">
            <div className="discovery-header" style={{marginBottom: '32px'}}>
                <h2 style={{fontSize: '24px', fontWeight: 700, margin: '0 0 8px'}}>Campus Canvas</h2>
                <p style={{color: 'var(--muted)', margin: 0}}>Explore top campuses and connect with active scouts.</p>
            </div>
            
            {loadingCampuses ? (
                <div style={{textAlign: 'center', padding: '48px'}}>
                    <p style={{color: 'var(--muted)'}}>Loading campuses...</p>
                </div>
            ) : (
                <div className="discovery-grid">
                    {campuses.map(campus => (
                        <div key={campus.id} className="campus-card">
                            ...
                            <p className="campus-vibe">{campus.vibe}</p>
                            <div className="campus-hooks">
                                {campus.insider_hooks.map((hook, i) => (
                                    <span key={i} className="hook-tag">{hook}</span>
                                ))}
                            </div>
                            
                            {campus.scouts.length > 0 && (
                                <>
                                    <div className="active-scouts-title">Active Scouts</div>
                                    <div className="scouts-list">
                                        {campus.scouts.map(scout => (
                                            <div key={scout.id} className="scout-item">
                                                <div className="scout-avatar">{scout.name.charAt(0)}</div>
                                                <div className="scout-info">
                                                    <span className="scout-name">{scout.name}</span>
                                                    <span className="scout-role">{scout.current_role_text}</span>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                            ...
                            <button 
                                className="campus-connect-btn"
                                onClick={() => handleWarmIntro(campus.name)}
                            >
                                Connect
                            </button>
                        </div>
